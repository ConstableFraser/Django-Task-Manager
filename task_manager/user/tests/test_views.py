...
class UserViewTestCase(TestCase):
    def setUp(self):
        ...
        self.athr = User.objects.create(first_name='Billy',
                                        last_name='Winky',
                                        username='Billy',
                                        password='supersecret#829'
                                        )
        self.status = Status.objects.create(name="status#1")
        self.task = Task.objects.create(name="task#1",
                                        status=self.status,
                                        author=self.athr)
    ...
    def test_view_user_invalid_update(self):
        ...
        self.assertEqual(cnt, 1)
        self.assertEqual(str(msg), _(USER_HVNT_PRMSSNS))
        self.assertEqual(response.status_code, 302)
